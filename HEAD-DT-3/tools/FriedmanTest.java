...
public class FriedmanTest {
    ...
    public static String Friedman(double[][] matriz, boolean asc) {
	...
	try {
	    ...
	    System.out.println("ChiFriedman: " + chiFriedman + " F_Friedman: " + F_Friedman + " df1: " + (nc - 1) + " df2:" + (nc - 1) * (nl - 1));
		...
		for (int jj = j + 1; jj < nc; ++jj) {
			...
			statsResults.append((sorts[nc - j - 1] + 1) + " " + (sorts[nc - jj - 1] + 1) + " " + (medRank[sorts[nc - j - 1]] - medRank[sorts[nc - jj - 1]]) + "\n");
		    }
		}
	    }
	    ...
	    /**
	     * algoritmo da coluna jj melhor que da linha j com x% de confianca
	     */
	    for (int j = 0; j < nc; ++j) {
		statsResults.append("\n," + (j + 1) + ",");
	...
	try {
		...
		System.out.println("Forma de uso: java -jar FriedmanTest.jar -f arquivo -t" +
			...
			"\n\nA opção -f realiza o teste de Friedman\n\n"+
	    ...
	    boolean tabelaVED = Utils.getFlag("t", args);
	    boolean friedman = Utils.getFlag("n", args);
	    boolean contrastEstimation = Utils.getFlag("c", args);
