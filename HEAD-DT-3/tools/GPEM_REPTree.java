...
public class GPEM_REPTree {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int base = 0; base < Dataset.getMetaTest().size(); base++) {
			status = false;
			for (CRparam = 2; CRparam <= 10; CRparam++) {
				...
				for (int fold = 0; fold < 10; fold++) {
					...
					if (CRparam >= training.numInstances()) {
						repTree.setNumFolds(training.numInstances() - 1);
						status = true;
					}
					else
						repTree.setNumFolds(CRparam);


					repTree.buildClassifier(training);
					Evaluation evalValidation = new Evaluation(validation);
					evalValidation.evaluateModel(repTree, validation);
					...
					evalTest.evaluateModel(repTree, test);
		...
		String summaryAllFile = dirResults + "Summary-REPTree.csv";
