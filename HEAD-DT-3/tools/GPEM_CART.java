...
public class GPEM_CART {
	...
	public static void main(String[] args) throws Exception {
			...
			for (CRparam = 2; CRparam <= 10; CRparam++) {
				...
				for (int fold = 0; fold < 10; fold++) {
					...
					if (CRparam >= training.numInstances()) {
						sCart.setNumFoldsPruning(training.numInstances() - 1);
						status = true;
					}
					else
						sCart.setNumFoldsPruning(CRparam);


					sCart.buildClassifier(training);
					Evaluation evalValidation = new Evaluation(validation);
					evalValidation.evaluateModel(sCart, validation);
					...
					evalTest.evaluateModel(sCart, test);
		...
		String summaryAllFile = dirResults + "Summary-CART.csv";
