...
implements Cloneable, Serializable, RevisionHandler {
	...
	/** Bag for missing values */
	private double m_missingBag[] = null;
	...
	public Distribution(Instances source, AbstractSplit modelToUse)
	throws Exception {
	...
	/**
	 * Adds all instances with unknown values for given attribute to all bags (see Friedman 1977)
	 *
	 * @exception Exception if something goes wrong
	 */
	public final void addInstWithUnknownToAll(Instances source,int attIndex)	throws Exception {
		...
		while (enu.hasMoreElements()) {
			...
			if (instance.isMissing(attIndex)) {
				...
				totaL = totaL+weight;
				for (j = 0; j < m_perBag.length; j++) {
					m_perClassPerBag[j][classIndex] = m_perClassPerBag[j][classIndex] + weight;
					m_perBag[j] = m_perBag[j] + weight;
				}
			}
		}
	}
	...
	/**
	 * Adds all instances with unknown values to the most probable bag (max bag) (see Quinlan 1989)
	 *
	 * @exception Exception if something goes wrong
	 */
	public final void addInstWithUnknownToMaxBag(Instances source,int attIndex)	throws Exception {

		int classIndex;
		Instance instance;
		double weight;
		double[] counts = new double[m_perClass.length];
		...
		while (enu.hasMoreElements()) {
			...
			if (instance.isMissing(attIndex)) {
				classIndex = (int)instance.classValue();
				weight = instance.weight();
				counts[classIndex] = counts[classIndex] + weight;
			}
		}
		add(maxBag(),counts);
	...
	/**
	 * Adds all instances with unknown values to the most probable bag (max bag) given the class of the instance (Loh and Shih 1997)
	 *
	 * @exception Exception if something goes wrong
	 */
	public final void addInstWithUnknownToMaxBagClass(Instances source,int attIndex) throws Exception {

		int classIndex;
		Instance instance;
		double weight;
		double[][] probs = new double[m_perClass.length][m_perBag.length];
		int mostProbableBag;
		int i,j;

		// Find out bag with largest number of instances from a given class (proportionally)
		for (j = 0; j < m_perBag.length; j++)
			for (i = 0; i < m_perBag.length; j++)
				probs[j][i] = prob(j,i);
		...
		while (enu.hasMoreElements()) {
			...
			if (instance.isMissing(attIndex)) {
				classIndex = (int)instance.classValue();
				weight = instance.weight();
				mostProbableBag = Utils.maxIndex(probs[classIndex]);
				m_perClass[classIndex] = m_perClass[classIndex]+weight;
				totaL = totaL+weight;
				m_perClassPerBag[mostProbableBag][classIndex] = m_perClassPerBag[mostProbableBag][classIndex] + weight;
				m_perBag[mostProbableBag] = m_perBag[mostProbableBag] + weight;
			}
		}
	}
	...
	/**
	 * Adds all instances with unknown values according to (un)supervised imputation.
	 *
	 * @exception Exception if something goes wrong
	 */
	public final void addInstWithUnknownImputation(AbstractSplit splitModel, Instances data, boolean supervised) throws Exception {
		Instance instance;
		int missingSet = 0;
		Enumeration<?> enu = data.enumerateInstances();
		while(enu.hasMoreElements()){
			instance = (Instance)enu.nextElement();
			if (instance.isMissing(splitModel.attIndex())) {
				if(supervised)
					missingSet  = splitModel.fillInstance(instance, data, true);
				else
					missingSet = splitModel.fillInstance(instance, data, false);

				add(missingSet,instance);
			}
		}
	}
	...
	/**
	 * Adds all instances with unknown values to a new bag (see Quinlan 1989)
	 *
	 * @exception Exception if something goes wrong
	 */
	public final void addInstWithUnknownToMissingBag(Instances source,int attIndex)	throws Exception {
		...
		while (enu.hasMoreElements()) {
			...
			if (instance.isMissing(attIndex)) {
				classIndex = (int)instance.classValue();
				weight = instance.weight();
				m_missingBag[classIndex] = m_missingBag[classIndex]+weight;
				totaL = totaL+weight;
			}
		}
	}
