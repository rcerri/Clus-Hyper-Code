...
public class myParetoEvaluator extends SimpleEvaluator {
	...
	public void evaluatePopulation(final EvolutionState state) {
		...
		for(int x = 0;x<state.population.subpops.length;x++) {
			for(int y = 0;y<state.population.subpops[x].individuals.length;y++) {
				...
				objectives[0] = ( ((myMultiObjectiveFitness)ind.fitness).measures.getfMeasure()[1] + ((myMultiObjectiveFitness)ind.fitness).measures.getfMeasure()[0] )/2 ; //f-measure
				objectives[1] = ( ((myMultiObjectiveFitness)ind.fitness).measures.getBalance()[1] + ((myMultiObjectiveFitness)ind.fitness).measures.getBalance()[0] )/2 ; // balance
				//objectives[1] = ((myMultiObjectiveFitness)ind.fitness).measures.getAvgClassAcc()[1] ;
				/*
