...
public class HMCConvertToSC {
	...
	public void convert2(String input, String output, boolean binary, boolean split, String tra, String tst) throws Exception {
		Clus clus = new Clus();
		String appname = FileUtil.getName(input)+".s";
		clus.initializeAddOn2(appname, tra,tst);
		ClusStatManager mgr = clus.getStatManager();
		Settings sett = clus.getSettings();
		ClassHierarchy hier = mgr.getHier();
		int sidx = hier.getType().getArrayIndex();
	...
	public static void main(String[] args, String tra, String tst) {
		int mainargs = 0;
		boolean binary = false;
		boolean split = false;
		boolean match = true;
		...
		HMCConvertToSC cnv = new HMCConvertToSC();
		try {
			cnv.convert2(input, output, binary, split,tra,tst);
		} catch (Exception e) {
			System.err.println("Error: "+e);
			e.printStackTrace();
		}
