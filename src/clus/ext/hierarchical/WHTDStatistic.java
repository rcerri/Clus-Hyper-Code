...
public class WHTDStatistic extends RegressionStatBinaryNomiss {
	...
	public void addPredictWriterSchema(String prefix, ClusSchema schema) {
		...
		// start addition celine
		schema.addAttrType(new NumericAttrType(prefix+"-reliability"));
		// end addition celine
	}
	...
	public void vote(ArrayList votes) {
		...
		double forestprediction;
		double targetsd;
		m_Reliability = 0.0;
		// for each target
		for (int i = 0; i < m_NbAttrs; i++){
			targetsd = 0;
			forestprediction = m_Means[i];
			// for each tree
			for (int j = 0; j < nb_votes; j++){
				treeprediction = ((WHTDStatistic) votes.get(j)).m_Means[i];
				targetsd += Math.pow((treeprediction-forestprediction),2);
			}
			targetsd /= (nb_votes-1);
			targetsd = Math.sqrt(targetsd);
			//System.out.println("target " + i + " -> sd " + targetsd);
			m_Reliability += targetsd;
		}
		m_Reliability /= m_NbAttrs;
		m_Reliability = 1.0 - m_Reliability;
		// end addition celine
		computePrediction();
	}
