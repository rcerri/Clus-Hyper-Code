...
public class myProblem extends Problem implements SimpleProblemForm {
		...
		try {
			...
			if (Main.mlTask > 0) { // classification
				...
				if (Main.mlTask == 2) { // multi-threads)
					...
					// initialization is done once, in MyEvolve.java
				...
				if( (currentGen == 0) || (currentGen == (state.numGenerations - 1)))  { // first job
					...
					if (currentGen == 0)
						Main.pFirstGen.println(measures.getAUROC()[0] +","+ measures.getAUPRC()[0] +","+ measures.getAUROC()[1] +","+ measures.getAUPRC()[1]);
					else {
						Main.pLastGen.println(measures.getAUROC()[0] +","+ measures.getAUPRC()[0] +","+ measures.getAUROC()[1] +","+ measures.getAUPRC()[1]);
					}
				}	
	...
	public void evaluateBestIndividual(IntegerVectorIndividual ind, EvolutionState state) throws Exception{
		...
		if (Main.mlTask > 0) {
			...
			if (Main.mlTask == 2) { // multi-threads)
				ClusWrapperNonStatic object = new ClusWrapperNonStatic();
				object.initialization(trainValidSet,valSet, Main.targets,Main.randomForest,true);
				measures = object.evaluateIndividualClassification(genome,true);
			}
			else { // ==1, single thread
				// true = classification or false = regression
				ClusWrapper.initialization(trainValidSet,valSet, Main.targets,Main.randomForest,true);
				measures = ClusWrapper.evaluateIndividualClassification(genome,true);
			}
