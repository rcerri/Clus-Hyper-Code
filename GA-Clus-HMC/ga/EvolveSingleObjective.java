		...
		for(int i = 0; i < Dataset.getNumFolds(); i++){
			...
			for(int job = currentJob ; job < numJobs; job++){
				try
				{
					...
					Main.fwParetoTest = new FileWriter(Main.path+"Fold"+i+"/ParetoFront/resultado-Pareto-fold"+i+"job"+job+".csv");
					Main.fwParetoAll = new FileWriter(Main.path+"Fold"+i+"/ParetoFront/resultado-Pareto-All-fold"+i+"job"+job+".csv");
					Main.pwParetoTest = new PrintWriter(Main.fwParetoTest);
					Main.pwParetoAll = new PrintWriter(Main.fwParetoAll);
					
					Main.pConfusionMatrixPareto = new PrintWriter(Main.path+"Fold"+i+"/ParetoFront/confusionMatrix-Pareto-fold"+i+"job"+job+".csv");
