 ...
 *
 */
public class ClusWrapperStatic {
	...
	private static InputStream createInputStreamBaseConfigFileForClassification(String target, boolean trainErrors) throws IOException{
		...
		if(target.endsWith(",")) target = target.substring(0, target.length()-1); // I remove the last comma.
		...
		if(target.equals(""))
		     cad += "\n[Hierarchical]\nType = TREE\n"; // target here is the targetInverted.
		else 
			cad += "\n[Hierarchical]\nType = TREE\nDisableLabels = "+target+"\n"; // target here is the targetInverted.
	...
	public static void runClassifier(String[] args, InputStream ConfigFile, boolean classification) throws IOException, ClusException{
		...
		// IN classification you don't need to modify the outputs.. they are always the same one. (Varies only the disableLabels)
	 ...
	 * If train == true, training errors are provided.
	 *
	 * 21/6/2016: this is a previous version for accuracy and F1 measure in classification data.
	...
	public static Double[][][] processOutputClassification(boolean train){
		...
		if(train){
			...
			while(!linea.contains("Original")){linea = lineas.nextToken();}     // To skip default result
		...
		for (int j=0; j<MaxClassifiers;j++){
			...
			if(!Classifier[j].equals("")){
				...
				if(PreviousSolutions.containsKey(Classifier[j])){ // if this is already in the HashTable:
					...
					// Run the classifier
					runClassifier(args, configFile,false);
		...
		for (int j=0; j<MaxClassifiers;j++){
			...
			if(!Classifier[j].equals("")){
				...
				if(PreviousSolutions.containsKey(Classifier[j])){ // if this is already in the HashTable:
					...
					// Run the classifier
					runClassifier(args, configFile,true);
